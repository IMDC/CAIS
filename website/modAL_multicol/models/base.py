...
class BaseLearner(ABC, BaseEstimator):
    ...
    def _fit_to_known(self, bootstrap: bool = False, **fit_kwargs) -> 'BaseLearner':
        ...
        Returns:
            self
        """
        # print("Boostratp@fit_to_known, base.py", bootstrap)
        if not bootstrap:
            self.estimator.fit(self.X_training, self.y_training, **fit_kwargs)
            # print(self.y_training[-1])
        else:
...
class BaseCommittee(ABC, BaseEstimator):
    ...
    def query(self, *query_args, **query_kwargs) -> Union[Tuple, modALinput]:
        ...
        if bool(self.queried_X): # if queried_X is not empty
            ...
            print("querying@base.py -shape:{} deleted:{}".format(new_X.shape, kl))
            query_result = self.query_strategy(self, new_X)
        else:
