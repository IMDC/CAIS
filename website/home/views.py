    ...
    if len(VIDEO_POOL) == 0:  # when video pool is empty
        ...
        if WAIT_SWITCH:
            ...
            if apps.get_app_config("home").count in MANUAL_IDX:
                ...
                CUR_PREDS = preds
                VIDEO_TITLE, CAPTION_TITLE = "/videos/test{}.mp4".format(
                    tmpidx
                ), "/captions/base_captions/test{}.vtt".format(tmpidx)
                print(
                    "Learn Count={} MANUAL_IDX={} MANUAL_COUNTER={} preds={}".format(
                        apps.get_app_config("home").count,
                        MANUAL_IDX,
                        MANUAL_COUNTER,
                        preds,
                    )
