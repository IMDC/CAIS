...
class ActiveLearningClient:
    ...
    def load_AL_models(self):
        ...
        if not Blobby.objects.exists():  # participant number == 1
            ...
            self.csv_url = settings.STATICFILES_DIRS[0] + "/um_{}.csv".format(
                20000
            )  # default, But MUST BE CHANGED for the actual learning.
            try:
            ...
            orig_urls, mod_urls = [
                dir_flag + "/originalfirst.h5",
                dir_flag + "/originalsecond.h5",
            ], [dir_flag + "/modifiedfirst.h5", dir_flag + "/modifiedsecond.h5"]
        ...
        return Committee(
            learner_list=learner_list, given_classes=np.array([1, 2, 3, 4, 5])
        )
    ...
    def get_data_for_hearing_group(self):
        ...
        q = Question.objects.filter(
            text="What statement best describes your relationship to the Deaf and/or Hard of Hearing Communities?"
        )[0]
