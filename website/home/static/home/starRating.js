...
const star_scale = {'0': "Click a star to teach Cappy", '1':"Strongly dissatisfied", '2':"Dissatisfied", '3':"Neither satisfied nor dissatisfied", '4':"Satisfied", '5':"Strongly satisfied"};
...
function resetStars(starsets) {
    ...
    for (blacklist in starname) {
        // This should always be true for the current starset...
        if (starname[blacklist] == starsets) {
            var allElems = document.getElementsByName(starname[blacklist]);
            for (let i = 0; i < allElems.length; i++) {
                allElems[i].style.color = "black";
            }
        }
    }
    var k = 0;
    for (const property in pp) {
        if (`${property}` == resetThisStar){
            ratingsArray[k] = defaultRatingsArray[k];
        }
        k+=1;
    }
...
function explainingStarRating(starid, value){
    starlabel_dom = "starexplain" + starid.slice(starid.length - 1);
...
function createStarList(value) {
    ...
    for (let i = 1; i < 6; i++) {
        ...
        if (i < value) {
            star.style.color = "rgb(226, 207, 37)";
...
function onStarExitListener(event) {
    ...
    if (prev_clicked_idx > 0){
        ...
        sibilingStars.forEach((star, index) => {
            star.style.color = (index <= new_idx)? "rgb(226, 207, 37)": "black";
        });
...
function onStarHoverListener(event) {
    ...
    sibilingStars.forEach((star, index) => {
        star.style.color = (index <= targetIndex)? "rgb(226, 207, 37)": "black";
    });
