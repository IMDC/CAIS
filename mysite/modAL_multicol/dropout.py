...
def mc_dropout_bald(
    ...
    dropout_layer_indexes: list = [],
    num_cycles: int = 50,
    sample_per_forward_pass: int = 1000,
    logits_adaptor: Callable[
        [torch.tensor, modALinput], torch.tensor
...
def mc_dropout_max_variationRatios(
    classifier: BaseEstimator,
    X: modALinput,
    n_instances: int = 1,
    random_tie_break: bool = False,
...
def get_predictions(
    ...
    ), "sample_per_forward_pass must be larger than zero"

    predictions = []
    # set dropout layers to train mode
    set_dropout_mode(
        classifier.estimator.module_, dropout_layer_indexes, train_mode=True
    )
...
def _mean_standard_deviation(proba: list) -> np.ndarray:
    ...
    mean_standard_deviation = np.mean(
        standard_deviation_class_vise,
        where=~np.isnan(standard_deviation_class_vise),
        axis=-1,
    )
