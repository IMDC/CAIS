...
def to_ms(captiontime):
    ...
    tmpms = tmpct[2].split(".")
    hour, minutes, second, milliseconds = (
        int(tmpct[0]),
        int(tmpct[1]),
        int(tmpms[0]),
        int(tmpms[1]),
...
def benchmark_prep(tr_vtt, cap_vtt):
    ...
    while i < len(tr_chunks):
        ...
        if c < len(cap_chunks):
            ...
            transcript_sentence, caption_sentence = nlp(tr_txt), nlp(
                cap_txt
            )  # lets compare the two sentences by using NLP
            use_sim = transcript_sentence.similarity(
                caption_sentence
            )  # use the similarity method that is based on the vectors, on Doc, Span or Token
            delay = to_ms(cap_vtt[cap_block[0]].start) - to_ms(
                tr_vtt[tr_block[0]].start
            )  # actual caption start time - transcript start time
            speed = get_block_avgwpm(cap_block[0], cap_block[1], cap_vtt)
            # speed = get_wpm(cap_vtt[cap_block[0]], cap_vtt[cap_block[1]], cap_block[2])
            if (
                use_sim < 0
            ):  # lets' consider the sentence is actually not correct at all
                ...
                paraphrasing = 1
                missing_words = find_error_words(
                    tr_txt, cap_txt
                )  # Find the number of missing words
                skipped = False
            else:
                skipped = False
            cb = cap_block[2]
        ...
        tr_msg = "[{} -> {}]: {}".format(
            tr_vtt[tr_block[0]].start, tr_vtt[tr_block[1]].end, tr_block[2]
        )
        cap_msg = "[{} -> {}]: {}".format(
            cap_vtt[cap_block[0]].start, cap_vtt[cap_block[1]].end, cap_block[2]
        )

        rep_values.append(
            [tr_msg, cap_msg, use_sim, delay, speed, missing_words, paraphrasing]
        )
...
def get_cappy_score(repvalues, scaler_x, flag=False):
    ...
    for r in rep_values:
        tr_b, cap_b, use_sim, delay, speed, mw, pf = (
            r[0],
            r[1],
            r[2],
            r[3],
        ...
        for hearing_condition in hearing_group:
            if delay > 0:
                val = scaler_x.transform(
                    np.asarray(
                        [[delay, speed, mw, pf, hearing_group[hearing_condition]]]
                    )
                )
            ...
            for idx in range(len(op)):
                ...
                ratingsentence += "{}:{} with {}\n".format(
                    caprates, cr[caprates], factors[idx]
                )
    ...
    for r in cappy_score_rack:
        ...
        for idx in range(len(op)):
            ...
            ratemsg += "- {:0.3f}/5, {} with the {}\n".format(
                caprates, cr[round(caprates)], factors[idx]
            )
            # ratemsg += "- {:0.3f} with the {}\n".format(caprates, factors[idx])
            # ratemsg += "- {} with the {} and {}\n".format(cr[round(caprates)], factors[idx], caprates)
        print(
            "Overall ratings by {} viewers might be:\n{}\nAverage can be:{}".format(
                r, ratemsg, np.mean(avgop)
            )
        )
    ...
    for r in rep_values:
        ...
        t_mw += r[5]
    print(
        "AVERAGE delay={:0.3f}, speed={:0.3f}, missing words/sentences={:0.3f}".format(
            t_d / len(rep_values), t_s / len(rep_values), t_mw / len(rep_values)
        )
    )
...
if __name__ == "__main__":
    caption_files = [
        "captions/GlobalWeather_A.vtt",
        "captions/GlobalWeather_B.vtt",
        "captions/NHL_A.vtt",
        "captions/NHL_B.vtt",
        "captions/SportsCentre_A.vtt",
        "captions/SportsCentre_B.vtt",
        "captions/TheSocial_A.vtt",
        "captions/TheSocial_B.vtt",
    ]

    transcript_files = [
        "transcripts/GlobalWeather_tc.vtt",
        "transcripts/NHL_tc.vtt",
        "transcripts/SportsCentre_tc.vtt",
        "transcripts/TheSocial_tc.vtt",
    ]
    ...
    # ## LOAD Cappy
    cappy = load_AL_models(
        "data_and_models/originalfirst.h5", "data_and_models/originalsecond.h5"
    )

    deaf_csv_url = "data_and_models/deaf_20000_used.csv"
    tmp_dataset = np.array(
        pd.read_csv(deaf_csv_url, header=None, sep=",")
    )  # actually need exception because it's i/o
